...
from dataprocess import get_data_loader_bert_prompt, VarianceLoss
...
def select_data_all(mem_set, proto_memory, config, model, divide_train_set, num_sel_data, current_relations,
                    selecttype):
    ####select data according to selecttype
    # selecttype is 0: cluster for every rel
    # selecttype is 1: use ave embedding
...
def train_model_with_hard_neg(config, model, mem_set, traindata, epochs, current_proto, tokenizer, ifnegtive=0,
                              threshold=0.2, use_loss5=True, only_mem=False):
    ...
    for epoch_i in range(epochs):
        ...
        typelabels, masks, mask_pos) in enumerate(data_loader):
            ...
            loss_var = varloss(logits, logits_proto)
            ...
            for index in memindex:
                ...
                if preindex in model.haveseenrelations:
                    ...
                    current_distrbution = model.mem_forward_update(model.prototypes[preindex].view(1, -1),
                                                                   model.bestproto)
            ...
            if len(memindex) == 0:
                ...
                loss = loss1 * lossesfactor1 + loss2 * lossesfactor2 + loss3 * lossesfactor3 + loss4 * lossesfactor4 + loss_var * lossesfactor7
            else:
                ...
                loss = loss1 * lossesfactor1 + loss2 * lossesfactor2 + loss3 * lossesfactor3 + loss4 * lossesfactor4 + loss5 * lossesfactor5 + loss6 * lossesfactor6 + loss_var * lossesfactor7
            loss.backward()
...
def train_memory(config, model, mem_set, train_set, epochs, current_proto, original_vocab_size, ifusemem=True,
                 threshold=0.2):
    train_set = []
    if ifusemem:
        mem_data = []
...
if __name__ == '__main__':
    ...
    for m in range(donum):
        ...
        modelforbase = proto_softmax_layer_bert_prompt(encoderforbase, num_class=len(sampler.id2rel),
        ...
        for i in range(6):  # 6 times different seeds to get average results
            ...
            ifnorm = True
            ##################################
            for steps, (training_data, valid_data, test_data, test_all_data_splited, test_all_data, seen_relations,
                        current_relations, _) in enumerate(sampler):
                ...
                current_proto = get_memory(config, modelforbase,
                                           proto_memory)  # 这时候的current_proto是根据81个关系的名称输入模型之中得到的81个fake embedding：[81, 200]
                ...
                for j in range(1):
                    ...
                    modelforbase = train_model_with_hard_neg(config, modelforbase, mem_set, training_data, epochs,
                                                             current_proto, encoderforbase.tokenizer, ifnegtive=0,
                                                             threshold=threshold, use_loss5=False)
                ...
                # add train memory
                current_proto = get_memory(config, modelforbase, proto_memory)
                modelforbase = train_memory(config, modelforbase, mem_set, training_data, epochs * 3, current_proto,
                                            original_vocab_size, True, threshold=threshold)
